...
case_a <- roll3 %>% 
  ...
  bind_rows(
    filter(
      select(roll3, x, y, mwc = mwc_take, emg = emg_take),
      x == 3, y == 2)) %>% 
...
df <- bind_rows(case_a, case_b)  

df %>% 
  ggplot(aes(x = mwc, y = emg, color = Case)) +
  geom_point() +
  #geom_point(aes(x = mwc_take, y = emg_take)) +
  geom_line() +
